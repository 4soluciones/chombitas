...
(function($){
	$.fn.bstreetable = function(options){
		...
		var settings = {
            ...
            text:{
                NodeDeleteText:"¿Estás seguro de que quieres eliminar esta categoría?"
            }
		};
		var TREENODECACHE = "treenode";
		var language ={};
		language.addchild = "Agregar subcategoría";
		if(options) {           
        ...
        /*render data*/
        var dom_addFirstLevel = $("<div class='tt-operation m-b-sm'></div>").append($("<button class='btn btn-sm btn-outline-secondary j-addClass'><i class='fa fa-level-down fa-3x'></i>&nbsp;Agregar categoría</button>"));
        ...
        element.delegate(".form-control","blur",function(){
            ...
            data.comment = parentUl.attr("data-comment");
            data.pinnercode = curElement.parents(".class-level-"+(parentUl.attr("data-level")-1)).children("ul").attr("data-comment");
            ...
            if(!data.id&&!curElement.attr("data-oldval")){
                ...
                settings.nodeaddCallback(data,function(_data){
                    if(_data){
                        ...
                        curElement.parent().parent().attr("data-comment",_data.comment);
        ...
        function renderHeader(_dom_header){
        	var dom_row = $('<div class="border border-secondary secondary-color"></div>');
    		...
    		dom_row.append($("<span class='textalign-center'>Operaciones</span>")); 
        ...
        function toggleicon(toggleElement){
        	...
        	if(_element.hasClass("fa-angle-down")){
        		_element.removeClass("fa-angle-down").addClass("fa-angle-up");
        		toggleElement.parent().addClass("selected");
        	}else{
        		_element.removeClass("fa-angle-up").addClass("fa-angle-down");
        		toggleElement.parent().removeClass("selected")
        	}
        }
		function toggleExpendStatus(curElement){
			if(curElement.find(".fa-angle-up").length>0){
        ...
        function generateTreeNode(curElement,row,curLevel,isPrepend){
            ...
            row.comment&&dom_ul.attr("data-comment",row.comment);
            if(curLevel-0>=settings.maxlevel){
                ...
                $('<li class="j-expend"></li>').append('<label class="fa fa-angle-down fa-2x purple-text rotate-icon p-xs"></label>').append($("<input type='text' class='form-control input-sm' required/>").attr("data-oldval",row['name']).val(row['name']).attr("name","name")).attr('data-id',row.id).appendTo(dom_ul);
            }
            ...
            if(settings.nodeaddEnable){
                if(curLevel-0>=settings.maxlevel){
                    ...
                    $("<li></li>").append($('<button class="btn btn-light btn-sm j-addChild"><i class="fa fa-plus fa-2x "></i>'+language.addchild +'</button>').attr("data-id",row.id)).appendTo(dom_ul);
                }
